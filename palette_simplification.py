...
def simplify_convex_palette(message_queue, points, target_vertices=10, max_iterations=500):
    """
    ...
    message_queue.put_nowait(("convex_hull", {'type': 'initial', 'vertices': current_vertices.tolist(), 'faces': current_faces.tolist()}))
    ...
    while iteration < max_iterations and len(current_vertices) > target_vertices:
        ...
        if not candidate_collapses:
            message_queue.put_nowait(("server_response", {'error': f"Aucune fusion possible à l'itération {iteration}", 'reset': True}))
            return None
        ...
        iteration += 1
        if iteration % 10 == 0:
            message_queue.put_nowait(("server_log", {"data": f"Iteration {iteration}: {len(current_vertices)} sommets"}))
    ...
    message_queue.put_nowait(("server_log", {"data": f"La simplification a pris {time.time() - t0:.2f} secondes."}))
