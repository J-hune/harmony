...
def extract_rgbxy_weights(message_queue, palette_rgb, image_orig):
    """
    ...
    asap_weights = compute_asap_weights_tan2016(message_queue, hull_rgb, palette_rgb)
    ...
    message_queue.put_nowait(("server_log", {"data": f"Le calcul des poids a pris {time.time() - t0:.2f} secondes"}))
    ...
    message_queue.put_nowait(("server_log", {"data": f"RMSE de reconstruction : {rmse:.2f}"}))

    # Envoi des poids par couche via socket
    for layer in range(mix_weights.shape[-1]):
        message_queue.put_nowait(("layer_weights", {
            "id": layer,
            "width": width,
            "height": height,
...
def compute_asap_weights_tan2016(message_queue, img_labels, tetra_palette):
    """
    ...
    uniq_weights = assign_face_weights(message_queue, uniq_labels, ordered_palette, hull, delaunay_test)
    ...
    message_queue.put_nowait(("server_log", {'data': f"Erreur maximale : {diff_val.max():.2f} (distance euclidienne)"}))
    message_queue.put_nowait(("server_log", {'data': f"Erreur médiane : {median(diff_val):.2f} (distance euclidienne)"}))
    message_queue.put_nowait(("server_log", {'data': f"RMSE : {rmse:.2f}"}))
...
def assign_face_weights(message_queue, uniq_labels, palette, hull_obj, delaunay_obj):
    """
    ...
    for simplex in hull_obj.simplices:
        if np.all(simplex != 0):
            ...
            try:
                ...
                continue
    if len(remaining) > 0:
        message_queue.put_nowait(("server_response", {'error': f"Erreur : {len(remaining)} pixels n'ont pas pu être assignés", 'reset': True}))
        return None
